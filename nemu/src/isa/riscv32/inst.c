...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , U, R(rd) = s->pc +4; imm = ((BITS(imm, 31, 31)<< 19 | BITS(imm, 19, 12) << 11 | BITS(imm, 20, 20)<< 10 | BITS(imm, 30, 21)) <<1); s->dnpc = s->pc + imm);
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Log("%x",imm); Mw(src1 + imm, 4, src2));
