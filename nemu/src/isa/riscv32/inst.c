...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  //这里注意需要符号位扩展
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = SEXT(Mr(src1 + imm, 1) & 0XFF, 8));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2) & 0xFFFF,16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2) & 0xFFFF);
  // INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = imm << 12);
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = SEXT(Mr(src1 + imm, 4) & 0xffffffff,32));

  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);

  // INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , B, Mw(src1 + imm, 1, src2));
  ...
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , B, Mw(src1 + imm, 4, src2));
