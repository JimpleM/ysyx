...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = ((int)src1 >= (int)src2) ? s->pc + imm : s->pc + 4);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, temp = s->pc +4;  s->dnpc = (src1+imm)& 0xffffffff; R(rd) =temp);

  //这里注意需要符号位扩展
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(Mr(src1 + imm, 2),16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2));
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(rd) = (int)imm << 12);
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4));

  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(rd) = src1 | src2);

  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , B, Mw(src1 + imm, 1, src2 & 0xff));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2 & 0xffff));
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(rd) = src1 << (src2 & 0x1f));
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(rd) = (unsigned int)src1 < (unsigned int)imm);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (((long long)src1*src2) >> 32) & 0xffffffff);
  ...
  //RV64I
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", slli   , I64, R(rd) = src1 << imm);       //有问题
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I64, R(rd) = (int)src1 >> imm);
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I64, R(rd) = (unsigned)src1 >> imm);
