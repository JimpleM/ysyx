...
//INSTPAT(模式字符串, 指令名称, 指令类型, 指令执行操作);
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli   , I64, R(rd) = src1 >> imm);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I64, R(rd) = (sword_t)src1 >> imm);
  INSTPAT("0000000 ????? ????? 010 ????? 00100 11", slti   , I64, R(rd) = (sword_t)src1 < (sword_t)imm);
  ...
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = ((long long)(sword_t)src1*(sword_t)src2 >> 32) & 0xffffffff);
