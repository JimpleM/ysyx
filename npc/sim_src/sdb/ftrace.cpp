...
void init_ftrace(const char *elf_file){
	...
	for (int i = 0; i < elf_hdr.e_shnum; i++){
		ReadDataFromFile(&buffer,sizeof(Elf32_Shdr), 1, elf_hdr.e_shoff+sizeof(Elf32_Shdr)*i, elf_fp);
		Elf32_Shdr* pShdr = (Elf32_Shdr *) buffer;
		// Search for symbol tab
		if (pShdr[0].sh_type == SHT_SYMTAB){
			memcpy(&sym_hdr, &pShdr[0], sizeof(Elf32_Shdr));
		}
		// Search for string tab
		if (pShdr[0].sh_type == SHT_STRTAB){
			memcpy(&str_hdr, &pShdr[0], sizeof(Elf32_Shdr));
			break;	// don't found shshtrtab
		}
	}
	...
	int number = sym_hdr.sh_size/sizeof(Elf32_Sym);
	for(int i=0; i<number; i++){
		ReadDataFromFile(&buffer, sizeof(Elf32_Sym), 1, sym_hdr.sh_offset+sizeof(Elf32_Sym)*i, elf_fp);
		Elf32_Sym* pSym = (Elf32_Sym *) buffer;
		if((pSym[0].st_info & 0x0f) == STT_FUNC){
			// printf("%x %d\n",pSym[i].st_value,pSym[i].st_name);
			memcpy(&func_trace[func_cnt++].symbol, &pSym[0], sizeof(Elf32_Sym));
		}
	}
