...
// __attribute__((used))确保该函数不会被编译器的优化移除，即使看起来似乎从不调用该函数。
static Token tokens[TOKENS_NUM] __attribute__((used)) = {};
static int nr_token __attribute__((used))  = 0;
// 利用rules对输入的表达式划分成一个个tokens
...
// 递归拆分tokens优先级，符号计算的优先度越低拆分的优先级越高，
...
*/
    ...
    /*
      If not minus or deref, continue to divide the expression by main operation
    */
